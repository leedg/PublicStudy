...
ODBCDatabase::ODBCDatabase() : environment_(SQL_NULL_HANDLE), connected_(false)
{
...
void ODBCConnection::open(const std::string &connectionString)
{
	...
	SQLRETURN ret = SQLDriverConnectA(
		connection_, nullptr, (SQLCHAR *)connectionString.c_str(), SQL_NTS,
...
bool ODBCConnection::isOpen() const { return connected_; }
...
ODBCStatement::ODBCStatement(SQLHDBC conn)
	: statement_(SQL_NULL_HANDLE), connection_(conn), prepared_(false),
		  timeout_(30)
{
...
void ODBCStatement::bindParameters()
{
	for (size_t i = 0; i < parameters_.size(); ++i)
	{
		SQLRETURN ret = SQLBindParameter(
			statement_, (SQLUSMALLINT)(i + 1), SQL_PARAM_INPUT, SQL_C_CHAR,
			SQL_VARCHAR, 0, 0, (SQLPOINTER)parameters_[i].c_str(),
			(SQLLEN)parameters_[i].length(), &parameterLengths_[i]);
...
void ODBCStatement::checkSQLReturn(SQLRETURN ret, const std::string &operation)
{
	if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
	{
		throw DatabaseException(
			operation + ": " + getSQLErrorMessage(statement_, SQL_HANDLE_STMT));
	}
}
...
void ODBCResultSet::loadMetadata()
{
	...
	for (SQLSMALLINT i = 0; i < columnCount; ++i)
	{
		...
		ret =
			SQLDescribeColA(statement_, (SQLUSMALLINT)(i + 1), columnName,
...
bool ODBCResultSet::isNull(size_t columnIndex)
{
	...
	SQLCHAR buffer[1];
	SQLRETURN ret =
		SQLGetData(statement_, static_cast<SQLUSMALLINT>(columnIndex + 1),
...
size_t ODBCResultSet::getColumnCount() const { return columnNames_.size(); }
