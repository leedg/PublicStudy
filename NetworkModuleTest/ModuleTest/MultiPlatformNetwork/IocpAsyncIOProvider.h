...
class IocpAsyncIOProvider : public AsyncIOProvider
{
  ...
  private:
	...
	{
		OVERLAPPED mOverlapped; // English: IOCP overlapped structure / ?쒓?:
								// IOCP ?ㅻ쾭??援ъ“泥?
	...
	HANDLE mCompletionPort; // English: IOCP completion port handle / ?쒓?: IOCP
							// ?꾨즺 ?ы듃 ?몃뱾
	std::map<SocketHandle, std::unique_ptr<PendingOperation>>
		mPendingOps; // English: Pending ops / ?쒓?: ?湲??묒뾽
	mutable std::mutex
		mMutex; // English: Thread safety mutex / ?쒓?: ?ㅻ젅???덉쟾??裕ㅽ뀓??
	ProviderInfo
		mInfo; // English: Provider info cache / ?쒓?: 怨듦툒???뺣낫 罹먯떆
