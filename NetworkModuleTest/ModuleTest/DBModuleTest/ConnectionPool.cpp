...
ConnectionPool::ConnectionPool()
	: initialized_(false), activeConnections_(0), maxPoolSize_(10),
		  minPoolSize_(2), connectionTimeout_(std::chrono::seconds(30)),
...
std::shared_ptr<IConnection> ConnectionPool::getConnection()
{
	...
	bool acquired = condition_.wait_for(
		lock, connectionTimeout_,
		[this]
		{
	...
	for (auto it = connections_.begin(); it != connections_.end();)
	{
		if (!it->inUse)
		{
			...
			if (idleDuration > idleTimeout_ &&
				connections_.size() > minPoolSize_)
			{
				it->connection->close();
				it = connections_.erase(it);
				continue;
