// epoll AsyncIOProvider test suite.
...
void TestEpollAssociateSocket()
{
    ...
    // Use a connected socketpair so AssociateSocket has a real fd.
    int fds[2];
...
void TestEpollSendRecv()
{
    ...
    // fds[1] = receiver side registered with epoll.
    ...
    // Write to fds[0] to trigger EPOLLIN on fds[1].
...
{
    // epoll does not support pre-registered buffers; verify the
