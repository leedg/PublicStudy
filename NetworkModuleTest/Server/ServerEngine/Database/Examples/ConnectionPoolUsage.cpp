...
void basicPoolExample() {
    ...
    try {
        ...
        std::cout << "Pool initialized with " << pool.getTotalConnections() << " connections" << std::endl;

        // Get connection from pool
        auto conn = pool.getConnection();
        std::cout << "Connection acquired, active: " << pool.getActiveConnections() << std::endl;
        ...
        std::cout << "Connection returned, active: " << pool.getActiveConnections() << std::endl;
...
void scopedConnectionExample() {
    ...
    try {
        ...
        std::cout << "Before scope: active = " << pool.getActiveConnections() << std::endl;

        {
            // Connection automatically returned when scope ends
            ScopedConnection scopedConn(pool.getConnection(), &pool);

            std::cout << "Inside scope: active = " << pool.getActiveConnections() << std::endl;
        ...
        std::cout << "After scope: active = " << pool.getActiveConnections() << std::endl;
...
void multiThreadedPoolExample() {
    ...
    try {
        ...
        auto workerFunc = [&pool](int threadId) {
            try {
                for (int i = 0; i < 3; ++i) {
                    ...
                    if (rs->next()) {
                        std::cout << "Thread " << threadId << " got count: " << rs->getInt(0) << std::endl;
                    }
        ...
        std::cout << "Final active connections: " << pool.getActiveConnections() << std::endl;
        std::cout << "Final total connections: " << pool.getTotalConnections() << std::endl;
...
void poolMonitoringExample() {
    ...
    try {
        ...
        auto printPoolStatus = [&pool]() {
            ...
            std::cout << "  Active: " << pool.getActiveConnections() << std::endl;
            std::cout << "  Available: " << pool.getAvailableConnections() << std::endl;
            std::cout << "  Initialized: " << (pool.isInitialized() ? "Yes" : "No") << std::endl;
        };
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            std::cout << "\nAfter acquiring connection " << (i + 1) << ":" << std::endl;
            printPoolStatus();
        }
        ...
        for (size_t i = 0; i < connections.size(); ++i) {
            ...
            std::cout << "\nAfter returning connection " << (i + 1) << ":" << std::endl;
            printPoolStatus();
        }
...
void poolConfigurationExample() {
    ...
    try {
        ...
        std::cout << "Query executed successfully with new settings" << std::endl;
...
int main() {
    ...
    std::cout << "\nNote: Uncomment examples to run with actual database" << std::endl;
