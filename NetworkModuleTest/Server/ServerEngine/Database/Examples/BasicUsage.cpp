...
void transactionExample() {
    ...
    try {
        ...
        try {
            ...
            stmt1->setQuery("INSERT INTO accounts (name, balance) VALUES (?, ?)");
            stmt1->bindParameter(1, "Account A");
            stmt1->bindParameter(2, 1000.0);
            stmt1->executeUpdate();

            auto stmt2 = conn->createStatement();
            stmt2->setQuery("INSERT INTO accounts (name, balance) VALUES (?, ?)");
...
void batchOperationExample() {
    ...
    try {
        ...
        std::cout << "Batch executed, " << results.size() << " operations completed" << std::endl;
