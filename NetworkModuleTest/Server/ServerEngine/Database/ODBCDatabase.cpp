...
namespace Database {
    ...
    {
        SQLRETURN ret = SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &mEnvironment);
        if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) 
        {
            throw DatabaseException("Failed to allocate ODBC environment handle");
        }
        
        ret = SQLSetEnvAttr(mEnvironment, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3, 0);
        if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) 
        {
            CleanupEnvironment();
    ...
    void ODBCConnection::Open(const std::string& connectionString)
    {
        ...
        SQLRETURN ret = SQLDriverConnectA(mConnection, nullptr,
    ...
    void ODBCStatement::BindParameters()
    {
        ...
        {
            SQLRETURN ret = SQLBindParameter(mStatement, (SQLUSMALLINT)(i + 1), SQL_PARAM_INPUT,
                                            SQL_C_CHAR, SQL_VARCHAR, 0, 0,
                                            (SQLPOINTER)mParameters[i].c_str(),
                                            (SQLLEN)mParameters[i].length(),
                                            &mParameterLengths[i]);
    ...
    std::unique_ptr<IResultSet> ODBCStatement::ExecuteQuery()
    {
        ...
        {
            BindParameters();
            SQLRETURN ret = SQLExecDirectA(mStatement, (SQLCHAR*)mQuery.c_str(), SQL_NTS);
    ...
    void ODBCStatement::CheckSQLReturn(SQLRETURN ret, const std::string& operation)
    {
        if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO)
        {
            throw DatabaseException(operation + ": " + GetSQLErrorMessage(mStatement, SQL_HANDLE_STMT));
        }
    }
    ...
    void ODBCResultSet::LoadMetadata()
    {
        ...
        for (SQLSMALLINT i = 0; i < columnCount; ++i)
        {
            ...
            ret = SQLDescribeColA(mStatement, (SQLUSMALLINT)(i + 1), columnName, (SQLSMALLINT)sizeof(columnName),
    ...
    bool ODBCResultSet::IsNull(size_t columnIndex)
    {
        SQLLEN indicator;
        SQLCHAR buffer[1];
        SQLRETURN ret = SQLGetData(mStatement, static_cast<SQLUSMALLINT>(columnIndex + 1), SQL_C_CHAR,
                                  buffer, 0, &indicator);
