...
void ODBCConnection::Open(const std::string &connectionString)
{
	if (mConnected)
	{
		return; // English: Already connected / ???: ???? ??®Îö≠???
	}
...
void ODBCStatement::AddBatch()
{
	...
	// ???: ?Ë¢Å‚ëπ???Î¨íÎàñ?????Îµ¨Ê≤É?éÏ±∏?ëÔΩáÎ™??ÑÏèÑ????????Í≥óÏ®Æ ?????????±Î≤â ??????Ë¢Å„Çã???Œª?ÉÁî±??
	if (!mQuery.empty())
	{
		BatchEntry entry;
...
std::vector<int> ODBCStatement::ExecuteBatch()
{
	...
	// ???: SQLExecDirectA?????ÑÏèÑ??????????ÎΩ?ê£???????àÎª¨???Å¬Ä???????Î•ÅÏ∂ø
	...
	for (auto &entry : mBatches)
	{
		...
		// ???: BindParameters() ?ÔßèÍæ™????Ë¢Å„Çã???ÑÏèÑ??????Îµ¨Ê≤É?éÏ±∏?ëÔΩáÎ™?mParameters???∞Í∑£Î≤??
		...
		// ???: ???±Î≤â ?ÑÏèÑ?????????Ë¢Å„Çã????£ÎÅâ????åÎêé???®Î∞¥Î¨??Œª?ÉÁî±??
...
void ODBCResultSet::Close()
{
	...
	// ???: Statement ?ÔßèÍªäÍµ?? ?Î¥î¬ÄÁ≠?statement???????®¬Ä?Í∑êÎê∞Ï≠?
}
