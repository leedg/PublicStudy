...
bool BaseMessageHandler::ProcessMessage(
...
std::vector<uint8_t> BaseMessageHandler::CreateMessage(
    Interfaces::MessageType type,
    Interfaces::ConnectionId connectionId,
    const void* data,
    size_t size)
{
...
bool BaseMessageHandler::RegisterHandler(Interfaces::MessageType type, MessageCallback callback) {
    if (!callback) {
        return false;
    }
...
bool BaseMessageHandler::ParseMessage(
    Interfaces::ConnectionId connectionId,
    const uint8_t* data,
    size_t size,
...
std::vector<uint8_t> BaseMessageHandler::SerializeMessage(const Interfaces::Message& message) {
...
Interfaces::MessageType BaseMessageHandler::GetMessageType(const uint8_t* data, size_t size) {
