...
namespace Network::AsyncIO::BSD
{
    ...
    private:
        ...
        int mKqueueFd;                           // English: kqueue file descriptor / 한글: kqueue 파일 디스크립터
        std::map<SocketHandle, PendingOperation> mPendingOps;  // English: Pending ops / 한글: 대기 작업
        std::map<SocketHandle, bool> mRegisteredSockets;  // English: Registered sockets / 한글: 등록된 소켓
