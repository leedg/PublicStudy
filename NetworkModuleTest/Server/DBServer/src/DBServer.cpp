...
namespace Network::DBServer
{
    ...
    bool DBServer::IsRunning() const
    {
        return mIsRunning;
    }
    
    void DBServer::SetDatabaseConfig(
    ...
    void DBServer::OnDataReceived(ConnectionId connectionId, const uint8_t* data, size_t size)
    {
        if (!data || size == 0)
            return;
                              ...
                              const void* data, size_t size)
    {
        ...
        // mAsyncIOProvider->SendAsync(socket, message.data(), message.size(), context);
    }
