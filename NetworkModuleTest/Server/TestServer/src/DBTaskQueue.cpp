...

{
    ...
    // English: Multi-worker is safe: sessionId % workerCount routes each session
    ...
    // English: Store injected database reference and create tables if DB
    // available
    ...
    // English: Start workers BEFORE WAL recovery so EnqueueTask() accepts
    // recovered tasks

    // 한글: WAL 복구 전에 워커 시작 - EnqueueTask()가 복구된 태스크를 받을 수
    // 있도록

    mIsRunning.store(true);

    // English: Create per-worker data structures first, then start threads.
...

    {
        ...
        mWorkers[i]->thread =
            std::thread(&DBTaskQueue::WorkerThreadFunc, this, i);
    }
...

{
    ...
    // English: Wake up every per-worker CV so threads can exit their wait loop.
    ...
    //          Threads are already joined so no lock contention, but we lock
    //          for consistency.
...

        {
...

            {
...

                {
                    ...
                    task.callback(false,
                                  std::string("Drain exception: ") + e.what());
                }
            }
        }
    }
...

{
    ...
    // English: Key-affinity routing: sessionId % workerCount.
...

    {
        ...
        // English: Re-check under queue lock to close the Shutdown() race
        // window.
...

        {
            ...
            // English: Increment global queue size counter (lock-free GetQueueSize)
...

        {
            ...
            // English: Task was WAL-pended but never queued. Mark done to avoid
            // replay.
...
void DBTaskQueue::RecordDisconnectTime(ConnectionId sessionId,
                                       const std::string &timestamp)
...
void DBTaskQueue::UpdatePlayerData(
    ConnectionId sessionId, const std::string &jsonData,
...

{
    ...
    EnqueueTask(
        DBTask(DBTaskType::UpdatePlayerData, sessionId, jsonData, callback));
...

{
    ...
    // Note: May be slightly inaccurate due to concurrent operations, but
...

{
    ...
    Logger::Info("DBTaskQueue worker[" + std::to_string(workerIndex) +
                 "] thread started");
...

    {
...

        {
...

            {
                ...
                // English: Decrement global queue size counter.
                // 한글: 글로벌 큐 크기 카운터 감소.
        ...
        // English: Process task outside of lock — DB I/O is blocking but does
...

{
...

    {
        ...
        default:
            ...
            result = "Unknown task type (SaveGameProgress and Custom are not "
                     "yet implemented)";

            Logger::Error("Unknown DB task type - SaveGameProgress and Custom "
...
bool DBTaskQueue::HandleRecordConnectTime(const DBTask &task,
                                          std::string &result)

{
    ...
    if (mDatabase->IsConnected())
    {
...

        {
            ...
            stmt->SetQuery(
                ...
                "INSERT INTO SessionConnectLog (session_id, connect_time) "
                "VALUES (?, ?)");
    ...
    Logger::Info("Session " + std::to_string(task.sessionId) +
                 " connected at " + task.data);
...
bool DBTaskQueue::HandleRecordDisconnectTime(const DBTask &task,
                                             std::string &result)
    ...
    if (mDatabase->IsConnected())
    {
...

        {
            ...
            stmt->SetQuery(
                ...
                "INSERT INTO SessionDisconnectLog (session_id, "
...
bool DBTaskQueue::HandleUpdatePlayerData(const DBTask &task,
                                         std::string &result)
...

{
    ...
    // English: mWalFile lazy-open is protected by mWalMutex to prevent race
...

{
    ...
    // English: Helper lambda — parse one WAL file (primary or .bak) into
    // pendingMap.

    //          Called for both files so that a crash during
    //          rename-and-re-enqueue does

    //          not permanently lose tasks that only appear in the backup.

    // 한글: 단일 WAL 파일(기본 또는 .bak)을 pendingMap으로 파싱하는 헬퍼 람다.

    //       rename+재인큐 도중 크래시 시 백업 파일에만 남은 태스크도 손실 없이
    //       복구.
...

    {
...

        {
...

            {
                ...
                {
                    sessionId =
                        static_cast<ConnectionId>(std::stoull(sessionStr));
                }
                catch (...)
                {
                    continue;
                ...
                // English: Higher-seq entry wins if both files have the same
                // seq.
...

    {
        ...
        // English: WAL existed but all tasks were completed — remove it (and
        // backup if any).
    ...
    //   1. Rename old WAL → backup  (atomic; preserves data if we crash
    //   mid-recovery)

    //   2. Re-enqueue each task      (WalWritePending appends PENDING to fresh
    //   mWalPath)

    //   3. Delete backup             (only after all tasks have new WAL
    //   entries)

    //

    //   On restart after a crash between steps 1 and 2: mWalPath is missing so
    //   the

    //   backup is checked first and tasks are recovered from it.

    //   On restart after a crash during step 2: mWalPath has the partial new
    //   entries;
...

    {
        ...
        // English: rename() failed (e.g. cross-device) — fall back to
        // delete-first.

        //          This is the pre-fix behaviour; acceptable since rename
        //          failure is rare.

        // 한글: rename() 실패(예: 크로스 디바이스) — 기존 삭제 우선 방식으로
        // 폴백.

        Logger::Warn("WAL: rename to backup failed, falling back to "
...

    {
        ...
        // English: Re-enqueue recovered task (walSeq=0 so it gets a new WAL
        // entry)
    ...
    // English: All recovered tasks now have fresh WAL entries in mWalPath —
    // safe to drop backup.
    ...
    Logger::Info("WAL: Recovered and re-queued " +
                 std::to_string(recoveredCount) + " task(s)");
}
