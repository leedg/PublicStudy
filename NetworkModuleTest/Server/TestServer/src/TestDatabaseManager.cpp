...
bool TestServerDatabaseManager::InitializeConnectionPool(
	const std::string &connectionString, int maxPoolSize)
{
	if (mIsInitialized)
	{
													   ...
													   const std::string &username)
{
	...
	try
	{
		...
		pStmt->SetQuery("INSERT INTO user_logins (user_id, username, "
						"login_time) VALUES (?, ?, CURRENT_TIMESTAMP)");
		...
		std::cout << "[TestServerDatabaseManager] User login saved: "
				  << username << " (rows: " << rowsAffected << ")" << std::endl;
	...
	uint64_t userId, const std::string &stateData)
{
	...
	try
	{
		...
		pStmt->SetQuery("UPDATE game_states SET state_data = ?, updated_at = "
