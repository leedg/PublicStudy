...
namespace Network::TestServer
{
    ...
    public:
        ...
        // English: Lifecycle
        // 한글: 생명주기
        bool Initialize(uint16_t port = Utils::DEFAULT_TEST_SERVER_PORT, const std::string& dbConnectionString = "");
        bool Start();
        void Stop();
    ...
    private:
        // English: Packet handler shared across all client sessions (stateless after ctor).
        //          Allocated once in TestServer ctor, injected into per-session OnRecv callbacks.
        // 한글: 모든 클라이언트 세션에서 공유하는 패킷 핸들러 (생성 후 stateless).
        ...
        // English: Asynchronous DB task queue — shared_ptr so the session configurator lambda
        ...
        // 한글: 비동기 DB 작업 큐 — 세션 설정 람다에서 weak_ptr 캡처를 위해 shared_ptr 사용.
