...
2. 서버 실행
   - `TestDBServer.exe -p 18002` (Windows 기본값, Linux/macOS는 8001)
   - `TestServer.exe -p 19010 --db --db-host 127.0.0.1 --db-port 18002` (Windows 기본값, Linux/macOS는 9000/8001)
3. 클라이언트 실행
   - `TestClient.exe --host 127.0.0.1 --port 19010` (Windows 기본값, Linux/macOS는 9000)

스크립트 실행 시 주의:
- PowerShell 실행 스크립트 기본 포트는 Windows 기준 `Server=19010`, `DB=18002`입니다.
- 포트가 이미 사용 중이면 자동으로 다음 빈 포트로 이동합니다. (강제 고정하려면 `-DisablePortFallback`)
