...
namespace Network::TestClient
{
    ...
    void LatencyStats::Update(uint64_t rtt)
    {
        ...
        // 한글: 이동 평균 계산
        avgRtt = ((avgRtt * pongCount) + static_cast<double>(rtt)) / (pongCount + 1);
        ++pongCount;
    }
    ...
    bool TestClient::Connect(const std::string& host, uint16_t port)
    {
        ...
        ret = connect(mSocket, addrResult->ai_addr, static_cast<int>(addrResult->ai_addrlen));
        freeaddrinfo(addrResult);
    ...
    void TestClient::NetworkWorkerThread()
    {
        ...
        while (!mStopRequested.load() && mState.load() == ClientState::SessionActive)
        {
    ...
    bool TestClient::RecvPacket(PacketHeader& outHeader, char* outBody, int bodyBufferSize)
    {
        ...
        {
            int bytesNeeded = static_cast<int>(sizeof(PacketHeader)) - mRecvBufferOffset;
            ...
            else
            {
                ...
                {
                    return false; // English: Timeout, not an error / 한글: 타임아웃, 에러 아님
                }
        ...
        // 한글: 헤더에서 패킷 크기 읽기
        const PacketHeader* pHeader = reinterpret_cast<const PacketHeader*>(mRecvBuffer);
        int packetSize = pHeader->size;

        if (packetSize < static_cast<int>(sizeof(PacketHeader)) || packetSize > MAX_PACKET_SIZE)
        {
        ...
        while (mRecvBufferOffset < packetSize)
        {
            ...
            else
            {
                ...
                if (err == WSAETIMEDOUT || err == WSAEWOULDBLOCK)
                {
                    return false; // English: Incomplete, try again later / 한글: 불완전, 나중에 재시도
                }
                Logger::Error("recv() failed: " + std::to_string(err));
                mState.store(ClientState::Disconnected);
                return false;
    ...
    void TestClient::ProcessPacket(const PacketHeader& header, const char* body)
    {
        ...
        case PacketType::SessionConnectRes:
        {
            ...
            HandleConnectResponse(reinterpret_cast<const PKT_SessionConnectRes*>(fullPacket));
            break;
        }
        ...
        default:
            Logger::Warn("Unknown packet type: 0x" +
                StringUtils::Format("%04X", header.id));
            break;
        }
    }
    ...
    ClientState TestClient::GetState() const
    {
        return mState.load();
    }
    ...
    bool TestClient::IsConnected() const
    {
        ...
        return state == ClientState::Connected || state == ClientState::SessionActive;
    }

    uint64_t TestClient::GetSessionId() const
    {
        return mSessionId;
    }
    ...
    void TestClient::RequestStop()
    {
        mStopRequested.store(true);
    }

    bool TestClient::IsStopRequested() const
    {
